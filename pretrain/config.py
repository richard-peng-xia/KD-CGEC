import torch
# transformer-base
d_model = 512
n_heads = 8
n_layers = 6
d_k = 64
d_v = 64
d_ff = 2048
# transformer-large
# d_model = 1024
# n_heads = 16
# n_layers = 6
# d_k = 64
# d_v = 64
# d_ff = 4096

dropout = 0.3
padding_idx = 0
bos_idx = 2
eos_idx = 3
src_vocab_size = 6609
tgt_vocab_size = 6609
batch_size = 64
epoch_num = 30
early_stop = 5
lr = 1e-7


# greed decode的最大句子长度
max_len = 60
# beam size for bleu
beam_size = 50
# Label Smoothing
use_smoothing = False
# NoamOpt
use_noamopt = True
# R2L re-ranking
num_nbest = 5


data_dir = './data'
train_data_path = './data/json/train_nlpcc.json'
dev_data_path = './data/json/dev_nlpcc.json'
test_data_path = './data/json/test_wiki200.json'
model_path = './experiment/nlpcc.bin'
log_path = './experiment/train_nlpcc.log'
output_path = './experiment/output.txt'

# gpu_id and device id is the relative id
# thus, if you wanna use os.environ['CUDA_VISIBLE_DEVICES'] = '2, 3'
# you should set CUDA_VISIBLE_DEVICES = 2 as main -> gpu_id = '0', device_id = [0, 1]
gpu_id = '0'
device_id = [0]

# set device
if gpu_id != '':
    device = torch.device(f"cuda:{gpu_id}")
else:
    device = torch.device('cpu')
